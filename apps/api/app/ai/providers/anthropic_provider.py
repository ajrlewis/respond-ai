"""Anthropic provider implementation through LangChain."""
...
class AnthropicProvider(AIProvider):
    """Anthropic chat provider; embeddings are intentionally unsupported."""
    ...
    def __init__(self, api_key: str) -> None:
        key = api_key.strip()
        if not key:
            raise AIConfigurationError("ANTHROPIC_API_KEY is required when using provider=anthropic.")
        self._api_key = key
    ...
    def _chat_model(self, *, model: str, temperature: float) -> Any:
        ...
        return ChatAnthropic(model=model, anthropic_api_key=self._api_key, temperature=temperature)
    ...
    async def acomplete(
        ...
        *,
        ...
        try:
            ...
            raise AIProviderError(f"Anthropic chat completion failed: {exc}") from exc
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                raise AIProviderError(f"Anthropic structured completion failed: {exc}") from exc
        except Exception as exc:
            raise AIProviderError(f"Anthropic structured completion failed: {exc}") from exc
        ...
        if parsed is None:
            raise AIProviderError(
                f"Anthropic structured completion returned no parsed payload for {response_schema.__name__}."
            )
